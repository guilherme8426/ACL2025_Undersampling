...
def load_splits_ids_pkl(folddir):
    splits = []
    
    with open(folddir, "rb") as splits_file:
        df_splits = pickle.load(splits_file)
    
    for line in df_splits.itertuples(index=False):
        train_index = list(line.train_idxs)
        test_index = list(line.test_idxs)
        splits.append((train_index, test_index))
    
    return splits

def load_splits_ids_for_is(args, DATAIN):
	old_splits = load_splits_ids(f"{DATAIN}/splits/split_{args.nfolds}.csv")
	# is_splits  = load_splits_ids(f"{args.path_selection}/{args.dataset}/split_{args.nfolds}_{args.ismethod}_idxinfold.csv")
	is_splits  = load_splits_ids_pkl(f"{args.path_selection}/{args.dataset}/split_{args.nfolds}_{args.ismethod}_idxinfold.pkl")
	splits = []
