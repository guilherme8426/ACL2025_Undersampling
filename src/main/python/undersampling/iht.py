...
class IHT(InstanceSelectionMixin):
    """ Edited Nearest Neighbours (IHT)
    Descrição:
    ==========
    ...
    def get_satrategy_array(self, y):
        ...
        #limit = min(contagem_classes[max_classe] - contagem_classes[min_classe], int(len(y)*0.5))
    ...
    def select_data(self, X, y):
        ...
        # allknn = InstanceHardnessThreshold(sampling_strategy=self.sampling_strategy)
        allknn = InstanceHardnessThreshold(random_state=0,
                                           estimator=LogisticRegression(solver='lbfgs', multi_class='auto'),
                                           sampling_strategy=self.sampling_strategy)
        
        allknn.fit_resample(X, y) 

        idx_s = allknn.sample_indices_
